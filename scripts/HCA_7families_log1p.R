...
#   3. Apply prespecified log1p transformations to absolute/count/duration indicators.
#   4. Run Hmisc::redun() from scratch at adjusted R^2 >= .95.
#   5. Cluster the retained variables empirically with ClustOfVar.
#   6. Cut the variable dendrogram at k = 7 families.
#   7. Represent each family by its first principal component.
#   8. For rows with missing family inputs, calculate the score from
#      observed standardized inputs and normalize by the observed
#      coefficient norm.
#   9. Standardize the seven family scores.
#  10. Run Euclidean / Ward.D2 country HCA in the 7D family-score space.
#  11. Evaluate k = 2,...,10 country-cluster solutions.
#  12. Bootstrap the top 3 country solutions with B = 1000.
#
...
cat("\nSCRIPT VERSION: HCA_7families_log1p_fixed — 2026-09-15\n")
...
# ============================================================
# 5. TRANSFORMATION AND STARTING-POOL DESCRIPTIVES
# ============================================================

# Missingness is described on the raw indicators.
...
X_red_raw <- X_all[, vars_reduced, drop = FALSE]
X_red <- X_transformed[, vars_reduced, drop = FALSE]
...
# ============================================================
#
# ClustOfVar groups the retained transformed variables according to their
# similarity structure. We do not force the old seven-family membership.
# ============================================================
...
# ============================================================
#
# Each family is summarized by its first PC fitted in the transformed
# indicator space on the same global complete-case training rows used above.
#
...
fit_family_scores <- function(
  ...
  for (family_name in names(families)) {
    ...
    # Extract PC1 coefficients and explicitly restore variable names.
    # This is important for singleton families, where matrix subsetting can
    # otherwise drop or alter names and v[first_var] may return NA.
    v <- as.numeric(
      pca$rotation[, 1]
    )

    rotation_names <- rownames(
      pca$rotation
    )

    if (is.null(rotation_names)) {
      rotation_names <- colnames(
        Z_train
      )
    }

    if (
      is.null(rotation_names) ||
      length(rotation_names) != length(v)
    ) {
      stop(
        paste0(
          "Could not recover PC1 coefficient names for family '",
          family_name,
          "'."
        )
      )
    ...
    names(v) <- rotation_names

    missing_coef_names <- setdiff(
      vars,
      names(v)
    )
    ...
    if (length(missing_coef_names) > 0) {
      stop(
        paste0(
          "PC1 coefficients are missing variable names in family '",
          family_name,
          "': ",
          paste(missing_coef_names, collapse = ", ")
        )
      )
    }

    if (any(!is.finite(v[vars]))) {
      stop(
        paste0(
          "Non-finite PC1 coefficient in family '",
          family_name,
          "'. Variables: ",
          paste(vars, collapse = ", ")
        )
    ...
    # Deterministic sign orientation: orient PC1 so the first listed
    # variable has a non-negative coefficient.
    first_var <- vars[1]
    first_coef <- unname(
      v[first_var]
    )
...
plot(
  ...
  cex = 0.55,
  hang = -1,
  main = "Country HCA in empirical seven-family score space — log1p preprocessing",
  xlab = "",
  sub = "",
  ylab = "Ward distance"
)
...
plot(
  var_tree,
  main = "Empirical clustering of retained transformed variables"
)
...
reproducibility_objects <- list(
  script_version =
    "HCA_7families_log1p_fixed — 2026-09-15",
  input_file =
...
cat("log1p-transformed variables:", length(vars_log1p), "\n")
