...
cat("\nSCRIPT VERSION: HCA_direct_25var_log1p — 2026-09-15\n")
...
# on their original scale. All 25 coordinates are z-standardized later.
vars_log1p <- c(
  "peak_mil_personnel_k",
  "peak_mil_population_k",
  "mil_personnel_latest_k",
...
# ============================================================
# 5. TRANSFORMATION, MISSINGNESS, AND MEDIAN IMPUTATION
# ============================================================

# Missingness is reported on the original/raw indicators.
...
# Median imputation is performed after transformation, in the same scale
# subsequently used for z-standardization and distance calculation.
X_imp <- X_transformed
...
for (v in vars_25) {
  ...
  if (any(miss)) {
    ...
    imputation_records[[v]] <- tibble::tibble(
      ...
      variable = v,
      transformation = ifelse(v %in% vars_log1p, "log1p", "none"),
      imputed_value_model_scale = med,
      imputed_value_raw_scale_equivalent = raw_equivalent
    )
...
plot(
  ...
  cex = 0.55,
  hang = -1,
  main = "Direct 25-variable HCA — prespecified log1p transformations",
  xlab = "",
  sub = "",
  ylab = "Ward distance"
)
...
readr::write_csv(
  transformation_map,
  file.path(OUT_DIR, "transformation_map.csv")
)

readr::write_csv(
  transformation_diagnostics,
  file.path(OUT_DIR, "transformation_diagnostics.csv")
)
...
analysis_settings <- tibble::tibble(
  ...
  value = c(
    ...
    MIL_EXP_SOURCE,
    "variable-wise median imputation after transformation",
...
reproducibility_objects <- list(
  script_version =
    "HCA_direct_25var_log1p — 2026-09-15",
  input_file =
